...
function createMockBenchmarkReport(metrics: Record<string, number>): BenchmarkReport {
...
describe('EvaluationReport', () => {
  it('生成包含所有模块的完整报告', () => {
    ...
    report.setRetrievalReport(createMockBenchmarkReport({ 'Recall@K': 0.8, 'Precision@K': 0.7 }));
    report.setGenerationReport(createMockBenchmarkReport({ Faithfulness: 0.9, BLEU: 0.4 }));
  ...
  it('检索指标低时生成改进建议', () => {
    const report = new EvaluationReport();
    report.setRetrievalReport(createMockBenchmarkReport({ 'Recall@K': 0.5 }));
  ...
  it('生成指标低时生成改进建议', () => {
    const report = new EvaluationReport();
    report.setGenerationReport(createMockBenchmarkReport({ Faithfulness: 0.6 }));
  ...
  it('综合评定：优秀', () => {
    const report = new EvaluationReport();
    report.setRetrievalReport(createMockBenchmarkReport({ 'Recall@K': 0.95 }));
  ...
  it('综合评定：需改进', () => {
    const report = new EvaluationReport();
    report.setRetrievalReport(createMockBenchmarkReport({ 'Recall@K': 0.3 }));
